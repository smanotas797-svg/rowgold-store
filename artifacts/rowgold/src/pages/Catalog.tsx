...
const SUBCATEGORY_MAP: Record<string, { label: string; subs: { key: string; label: string }[] }> = {
  joyas: {
    label: "Joyería",
    subs: [
    ...
    <div className="min-h-screen pt-24 pb-20 px-6" style={{ background: "#060606" }}>
      <div className="max-w-7xl mx-auto">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-14">
          <p className="text-[10px] tracking-[0.45em] uppercase mb-4" style={{ color: "rgba(212,175,55,0.55)" }}>ROWGOLD</p>
          <h1 style={{
          ...
          {selectedSub && (
            <p className="text-sm mt-2 capitalize" style={{ color: "rgba(212,175,55,0.5)", letterSpacing: "0.1em" }}>
              {selectedSub}
            </p>
          )}
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-6">
          <div className="relative flex-1">
            <Search size={13} className="absolute left-4 top-1/2 -translate-y-1/2" style={{ color: "rgba(212,175,55,0.4)" }} />
            <input
              type="search"
              ...
              style={{ border: "1px solid rgba(212,175,55,0.18)", color: "rgba(255,255,255,0.8)", letterSpacing: "0.04em" }}
          ...
          <select
            ...
            style={{ border: "1px solid rgba(212,175,55,0.18)", color: "rgba(212,175,55,0.6)", minWidth: 190, background: "#0a0a0a" }}
          >
            <option value="default"    style={{ background: "#111" }}>Ordenar por</option>
            ...
            <option value="name"       style={{ background: "#111" }}>Nombre A-Z</option>
            <option value="rating"     style={{ background: "#111" }}>Mejor Valorados</option>
          </select>
        </div>
        ...
        <div className="mb-4">
          <div className="flex flex-wrap gap-2 mb-3">
            <FilterPill active={selectedCategory === ""} onClick={() => handleCategoryChange("")} label="Todos" testId="filter-all" />
            {(categories ?? []).map((cat) => (
              <FilterPill
                key={cat.id}
        ...
        <div className="flex items-center justify-between mb-8">
          <p className="text-xs" style={{ color: "rgba(255,255,255,0.22)", letterSpacing: "0.1em" }}>
            {filtered.length} {filtered.length === 1 ? "pieza encontrada" : "piezas encontradas"}
            {selectedSub && <span className="ml-2 capitalize" style={{ color: "rgba(212,175,55,0.4)" }}>en {selectedSub}</span>}
          </p>
          {(search || selectedCategory || selectedSub) && (
            <button
              onClick={() => { setSearch(""); setSelectedCategory(""); setSelectedSub(""); }}
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
            {Array.from({ length: 8 }).map((_, i) => (
              <div key={i} className="luxury-card animate-pulse" style={{ aspectRatio: "3/4" }} />
            ))}
          </div>
        ) : filtered.length === 0 ? (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-28">
            <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "1.5rem", color: "rgba(255,255,255,0.18)" }}>
              No se encontraron piezas
            </p>
            <button
              onClick={() => { setSearch(""); setSelectedCategory(""); setSelectedSub(""); }}
              className="mt-6 px-8 py-3 text-xs tracking-[0.25em] uppercase transition-all"
              style={{ border: "1px solid rgba(212,175,55,0.25)", color: "rgba(212,175,55,0.6)" }}
            >
              Ver Todo
            </button>
          </motion.div>
