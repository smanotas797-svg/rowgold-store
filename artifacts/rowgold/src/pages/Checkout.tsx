...
const schema = z.object({
  ...
  paymentMethod: z.enum(["mercadopago", "paypal", "stripe", "transferencia", "contra_entrega"]),
});
...
export default function Checkout() {
  ...
  const createOrder = useCreateOrder({ request: { headers: { "x-session-id": sessionId } } } as Parameters<typeof useCreateOrder>[0]);
      ...
      <div className="min-h-screen pt-24 flex items-center justify-center" style={{ background: "#080808" }}>
        ...
        >
          <motion.div
            ...
            style={{ border: `2px solid ${GOLD}`, background: "rgba(212,175,55,0.05)" }}
          >
    ...
    <div className="min-h-screen pt-24 pb-20 px-6" style={{ background: "#080808" }}>
      <div className="max-w-5xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-12">
          <p className="text-xs tracking-[0.35em] uppercase mb-3" style={{ color: GOLD }}>Ultimo Paso</p>
          <h1
            style={{
              fontFamily: "'Cormorant Garamond', serif",
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="luxury-card p-8">
                <h3 className="mb-6 text-xs tracking-[0.25em] uppercase" style={{ color: "rgba(212,175,55,0.7)" }}>
                  Informacion de Envio
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  {[
                    { name: "name" as const, label: "Nombre Completo", placeholder: "Juan Garcia" },
                    { name: "email" as const, label: "Email", placeholder: "juan@email.com" },
                  ].map((field) => (
                    <div key={field.name}>
                  ...
                  <div className="md:col-span-2">
                    ...
                    {form.formState.errors.address && (
                      <p className="text-xs mt-1" style={{ color: "#ef4444" }}>{form.formState.errors.address.message}</p>
                    )}
                  </div>
                  {[
                    { name: "city" as const, label: "Ciudad", placeholder: "Ciudad de Mexico" },
                    { name: "country" as const, label: "Pais", placeholder: "Mexico" },
                  ].map((field) => (
                    <div key={field.name}>
                      <label className="block text-[10px] tracking-widest uppercase mb-2" style={{ color: "rgba(212,175,55,0.5)" }}>
                        {field.label}
                      ...
                      />
                      {form.formState.errors[field.name] && (
                        <p className="text-xs mt-1" style={{ color: "#ef4444" }}>{form.formState.errors[field.name]?.message}</p>
                      )}
                    </div>
                  ))}
                </div>
                ...
                <div className="space-y-3">
                  {paymentMethods.map((method) => (
                    ...
                    >
                      ...
                      <span className="text-sm" style={{ color: form.watch("paymentMethod") === method.value ? GOLD : "rgba(255,255,255,0.5)" }}>
                        {method.label}
                      </span>
                    </label>
                  ))}
            ...
            <div>
              <div className="luxury-card p-8">
                ...
                <div className="space-y-3 mb-6">
                  {(cart?.items ?? []).map((item) => (
                    <div key={item.productId} className="flex justify-between text-sm" style={{ color: "rgba(255,255,255,0.5)" }}>
                ...
                <div className="space-y-2 mb-6">
                  ...
                  <div className="flex justify-between text-sm" style={{ color: "rgba(255,255,255,0.4)" }}>
                    <span>Envio</span>
                    <span>{(cart?.shipping ?? 0) === 0 ? "Gratis" : `$${cart?.shipping}`}</span>
                  </div>
                  <div className="flex justify-between">
                    <span style={{ fontFamily: "'Cormorant Garamond', serif", color: "rgba(255,255,255,0.8)" }}>Total</span>
                    <span style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "1.3rem", color: GOLD }}>
                ...
                <div className="flex items-center justify-center gap-2 mt-4">
                  ...
                  <p className="text-[10px] tracking-wider" style={{ color: "rgba(255,255,255,0.25)" }}>
