...
import { db, reviewsTable, productsTable } from "../db/index.js";
...
router.post("/products/:id/reviews", async (req, res) => {
  try {
    ...
    const [review] = await db.insert(reviewsTable).values({
      productId, authorName: body.authorName, authorEmail: body.authorEmail ?? null,
      rating: body.rating, title: body.title ?? null, body: body.body, verified: 0,
    }).returning();
    const stats = await db.select({ avgRating: avg(reviewsTable.rating), totalCount: count(reviewsTable.id) })
