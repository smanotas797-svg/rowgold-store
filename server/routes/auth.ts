...
router.post("/auth/register", async (req, res) => {
  try {
    ...
    res.status(201).json({
      user: { id: user.id, email: user.email, username: user.name, role: user.role, createdAt: user.createdAt.toISOString() },
    });
  } catch (err) {
...
router.get("/auth/me", async (req, res) => {
  try {
    ...
    res.json({ id: user.id, email: user.email, username: user.name, role: user.role, createdAt: user.createdAt.toISOString() });
  } catch (err) {
